...
# Accessing Spotify tokens and secret key
...
# For the selected track/song getting name, artist, track id, track album, track image
    ...
    if len(tracks_list) > 0:
        for track in tracks_list:
            ...
            if str_temp == selected_track:
                ...
                # print(img_album)
                # saving album image with track id in the img directory
                songrecommendations.save_album_image(img_album, track_id)
    selected_track_choice = None
    if track_id is not None:
        image = songrecommendations.get_album_image(track_id)
        # showing the image on the streamlit
        st.image(image)
        # On the sidebar getting drop down options for Song Features & Similar Songs Recommendations
        ...
        if selected_track_choice == 'Song Features':
            ...
            # Creating dataframe with all the features of the selected song/track
            df = pd.DataFrame(track_features, index=[0])
            # Selecting only desired columns for the dataframe on the app
            df_features = df.loc[:,
                          ['acousticness', 'danceability', 'energy', 'instrumentalness', 'liveness', 'speechiness',
                           'valence']]
            # Showing dataframe on teh app
            st.dataframe(df_features)
            # Showing the polar-plot on teh app with song features.
            polarplot.feature_plot(df_features)
        # if similar songs recommendation is selected then
            ...
            recommendation_list_df = pd.DataFrame(recommendation_list).sort_values(by="popularity", ascending=False)
...
# If selected choice on the sidebar is album
    ...
    if album_id is not None and album_uri is not None:
        ...
        for idx in df_tracks_min.index:
            with st.container():
                # It will create 4 columns - for Id, Name, Duration in ms, Explicit
...
# If Artist is selected in teh sidebar
if selected_artist is not None and len(artists) > 0:
    ...
    if artist_id is not None:
        # Will create a dropdown with two options - Albums & Top Songs
    ...
    if selected_artist_choice is not None:
        if selected_artist_choice == 'Albums':
            ...
            # Three cols will be created with name, release date, total tracks in teh album
        ...
        # If top songs is selected
           ...
           # st.write(top_songs_result["tracks"])
            for track in top_songs_result['tracks']:
                with st.container():
                    ...
                    with col4:
                        ...
                        similar_songs_state = st.button('Similar Songs', key=track['id']+track["name"],
